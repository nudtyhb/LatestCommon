...
public  abstract class AbstractInstruction implements Instruction {
    private static int nextIndex = 0;  // used to check the overflow of instruction index
   // map from class to integer;   size added as elements put, so the map maps the boundary of each class!
    ...
    static { 
        ...
        instructions.put(LabelMarker.class, instructions.size());  // what is the concrete meaning of marker of jump target?
        ...
        instructions.put(SimpleInstruction.class, instructions.size());  // NOP, DUP, ADD_, CONST_
        instructions.put(TableSwitchInstruction.class, instructions.size());
        instructions.put(TypeInstruction.class, instructions.size());
        instructions.put(VarInstruction.class, instructions.size()); // a local variable instruction , load, store, RET
    }
    ...
    private final int index; // the index in the biteCode trace
   ...
   public  AbstractInstruction(final ReadMethod readMethod, final int opcode, final int lineNumber, final int index) {
    ...
    public static AbstractInstruction readFrom(final DataInputStream in, final MethodReadInformation methodInfo,
        ...
        final byte type = in.readByte(); // the type then used to get the method from readMethods
    ...
    // 由于逆向扫描是一步一步获取要处理的指令，此时，由要处理的指令来提取具体要处理的指令实例！，即当前指令对应的指令实例！
    @Override
	public <InstanceType> InstanceType getNextInstance(
    	...
    	// 其实还是返回当前的实例，但是指令index对应的OccurrenceNumber变得增加了1，因为新增加了一个实例。
    ...
    // 同一个方法中的前一条指令 除了 index-1!= instructionStart
    @Override
 ...
 // 同一个方法中的后一条指令 除了 index+1!= instructionEnd
    @Override
