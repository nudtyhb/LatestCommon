 ...
 */
public class LookupSwitchInstruction2 extends AbstractInstruction {

    private int defaultHandler;
    private Map<Integer,Integer> handlers;

    public LookupSwitchInstruction2(final ReadMethod readMethod, final int lineNumber,
            final int defaultHandler, final Map<Integer,Integer> handlers) {
    ...
    public LookupSwitchInstruction2(final ReadMethod readMethod, final int lineNumber,
            final int defaultHandler, final Map<Integer,Integer> handlers, final int index) {
        super(readMethod, Opcodes.LOOKUPSWITCH, lineNumber, index);
        this.defaultHandler = defaultHandler;
        this.handlers = handlers;
    }

    public int getDefaultHandler() {
        return this.defaultHandler;
    }

    public void setDefaultHandler(final int defaultHandler) {
        this.defaultHandler = defaultHandler;
    }

    public Map<Integer,Integer> getHandlers() {
        return this.handlers;
    }

    public void setHandlers(final Map<Integer,Integer> handlers) {
        this.handlers = handlers;
    }
...
*/
    ...
    public String toString() {
        final List<Entry<Integer, Integer>> sortedHandlers =
            new ArrayList<Entry<Integer,Integer>>(this.handlers.entrySet());
        Collections.sort(sortedHandlers, new Comparator<Entry<Integer, Integer>>() {
            @Override
        ...
        boolean first = true;
        for (final Entry<Integer, Integer> entry : sortedHandlers) {
            if (first)
                first = false;
            else
                sb.append(", ");
