...
public class ReadClass implements Comparable<ReadClass> {
    ...
    public int instructionNumberEnd;
    private final String source;
    private final int access;
    private final String superClassName;

    // instruct ReadClass, need to get the internalClassName(String) , the instructionNumStart(Int), int access, sourceFile(String), List<Filed> fields, String SuperClassName. 
    // Six components
    ...
    public ReadClass(final String internalClassName, final int instructionNumberStart,
            ...
            final String superClassName,int label) {
    ...
    public void ready() {
        this.methods.trimToSize(); // cut the useless storage
    }
    ...
    } // output the info of the read class
   
    // ReadFrom has DataInputStream in and StringCache. 
    // internalClassName =StringCache.readstring(in)
    // instructionNumStart=OptimizedDataInputStream.readInt0(in)
    // access=OptimizedDataInputStream.readInt0(in)
    // source=stringCache.readString(in)
    // superClass=StringCache.readString(in)
    // numFields=OptimizedDataInputStream.readInt0(in)
    // fields=Collections.emptyList(numFields);
    // readClass=new (six...) 
    // initial the fields by fields.add(Field.readFrom(in,stringCache,readClass);
    // NumMethods=OptimizedDataInputStream.readInt0(in)
    // ReadMethod=ReadMethod.readFrom(in,ReadClass,instrIndex,StringCache)) instrIndex is the start of one method
 //   static int method_count=0;
    ...
    public static ReadClass readFrom(final DataInputStream in, final StringCacheInput stringCache) throws IOException {
        ...
        final ReadClass rc = new ReadClass(intName, instructionNumberStart, access, source, fields, superClass); //use the info of input stream to initial the Read class
        while (numFields-- > 0)
  ...
  //      method_count+=numMethods;
        ...
        //   System.out.println("Current Method num and  begin index:  ");
       //    System.out.print(method_count);
        //  System.out.println("     ---     ");
      //      System.out.print(instrIndex);
     //      System.out.println("\n");
	...
	public int compareTo(final ReadClass o) {
        return this == o ? 0 : this.className.compareTo(o.className);   // string compare
    }
