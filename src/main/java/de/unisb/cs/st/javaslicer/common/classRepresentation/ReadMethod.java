...
public class ReadMethod implements Comparable<ReadMethod> {
    ...
    public AbstractInstruction methodenter;
    ...
    public int getEntryIndexLabel(){
    	return this.EntryIndexLabel;
    }
    ...
    public static ReadMethod readFrom(final DataInputStream in, final ReadClass readClass, final int instructionNumberStart,
        ...
        // 因为执行的指令有 可能一条 指令执行多次 ，所以导致 以numInstr作为容量会 偏大！
        // 现将LabelMarker型指令单独提取出来，装换成LabelMarker型，然后按照 index从小到大和普通指令一样插入到instructions中！要求指令不能重复插入！
        ...
        // 存放规则应该是Method中的LabelMarker型的放到前面（LabelMarker 也是按照从index小到大读），
        //所以先读LabelMarker, 直到读到第一个非labelMarker的正常指令，此时退出第一个while循环！
        while (numInstr-- > 0) {
            ...
            labels.add(lm);
            mri.labels.put(lm.getLabelNr(), lm); //mri.labels是一个Map: labelNr---> labelMarker!!!!  每一个方法一个！！
            instr = null;
        }
    //第二个循环是从正常指令开始扫描，instr==null才扫描视为了防止上面的while的最后读的正常指令丢失！
